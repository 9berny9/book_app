...
def main(books, ratings):
    ...
    col3.metric(label="Number of Ratings", value=f"""{len(ratings['User-ID'])}""")

    user_name = st.text_input("Enter your name:")

    #language_condition = br.languages[language]
    ...
    if user_name:
        ...
        if select_genres:
            ...
            if publisher_language:
                book_author = st.text_input("Author's name:")
                #book_author = st.selectbox(f"Select your favorite author:", c.get_language(books, publisher_language)['Book-Author'])
                if book_author:
                    ...
                    if worst_box:
                        ...
                        book_list_corr = result[1][0]['book'].to_list()
                        recommendation(book_list_corr)
